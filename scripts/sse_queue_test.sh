...
# ==================== 配置参数 ====================
BASE_URL="${BASE_URL:-http://localhost:9090/api/koawa-agent}"
...
for i in $(seq 1 "${CONCURRENCY}"); do
  (
    ...
    curl -G -N --no-buffer \
      ...
      > "${LOG_DIR}/koawa_agent_chat_${i}.log"
...
for i in $(seq 1 "${CONCURRENCY}"); do
  LOG_FILE="${LOG_DIR}/koawa_agent_chat_${i}.log"
  if [[ -f "${LOG_FILE}" ]]; then
    SIZE=$(du -h "${LOG_FILE}" | cut -f1)
    LINES=$(wc -l < "${LOG_FILE}")
    echo -e "   ${GREEN}✓${RESET} ${GRAY}koawa_agent_chat_${i}.log${RESET} ${DIM}(${SIZE}, ${LINES} lines)${RESET}"
  else
    echo -e "   ${RED}✗${RESET} ${GRAY}koawa_agent_chat_${i}.log${RESET} ${RED}[Missing]${RESET}"
  fi
done
