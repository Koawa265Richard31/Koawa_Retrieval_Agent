...
class TimeBoostPostProcessorTest {
    ...
    private final TimeBoostPostProcessor processor = new TimeBoostPostProcessor(helper, props);
    ...
    @Test
    void processShapesCandidatePoolToTopKMultiplier() {
        SearchContext ctx = SearchContext.builder().topK(2).build();
        List<RetrievedChunk> out = processor.process(chunksWithTime(20, 10), List.of(), ctx);
        assertThat(out).hasSizeLessThanOrEqualTo(2 * props.getRerankCandidateMultiplier());
